...
| `OpenAiLlmService` | 呼叫 GitHub Models `gpt-4o-mini` streaming（相容 OpenAI Chat Completions 格式） | ❌（需 GitHub PAT） | 免費雲端展示 |
...
│   │   ├── FakeLlmService.cs
│   │   ├── OllamaLlmService.cs
│   │   └── OpenAiLlmService.cs             # GitHub Models GPT-4o-mini（相容 OpenAI Chat API）
│   ├── Embedding/
...
| **改用 GitHub Models GPT-4o-mini** | 免費方案（每日 150 次請求），超量只回傳 429 不扣費；`BaseUrl` 指向 `https://models.inference.ai.azure.com`，API 格式與 OpenAI Chat Completions 相容，只需替換 GitHub PAT 作為 ApiKey |
...
### 2026-04-02：改用 GitHub Models GPT-4o-mini 作為雲端 LLM Provider

- **原因**：OpenAI API 需付費且綁定信用卡；GitHub Models 提供免費的 GPT-4o-mini 存取（每日 150 次請求），超量僅回傳 `429`，不產生費用，適合 Demo 展示場景
- **決策**：`OpenAiLlmService.cs` 實作 GitHub Models endpoint；`appsettings.json` 新增 `OpenAi.BaseUrl = "https://models.inference.ai.azure.com"`，`Model` 改為 `gpt-4o-mini`；以 GitHub Personal Access Token 作為 ApiKey
- **影響**：`LlmSettings` 新增 `OpenAi.BaseUrl` 欄位；`OpenAiLlmService` 從空殼改為完整實作；Provider 切換仍僅需修改 `appsettings.json`，業務邏輯零改動
